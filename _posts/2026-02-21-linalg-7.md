...
> $$||U\mathbf{x}|| = ||\mathbf{x}|| \quad \text{and} \quad (U\mathbf{x}) \cdot (U\mathbf{y}) = \mathbf{x} \cdot \mathbf{y}$$
...
The intuition is simple: pick vectors one by one and subtract their projections onto the space spanned by the previously selected vectors. This ensures each new vector is orthogonal to the ones before it.
...
SVD tells us that any linear transformation can be decomposed into a rotation ($V^T$), a scaling ($\Sigma$), and another rotation ($U$). This is how AI models like PCA or recommendation systems extract the most important "directions" from high-dimensional data.
